...
当前列表共 66 条。每条记录的端口直接取自候选表对应行的 `quic_port_udp` 列，本批次
该列（以及对应的 `tcp_port` 列）均为 443；导入列表时没有再次对这些端口逐条做独立
网络实测。部署脚本会在实际使用前从 VPS 依次验证端口可达性、QUIC/HTTP/3 支持及
HTTP 状态码，任何验证失败都不会使用该候选。
